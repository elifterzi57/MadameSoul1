...
export const LogsTab: React.FC<LogsTabProps> = ({ userRole: _userRole }) => {
  ...
  const fetchLogs = async (showLoading = true) => {
    ...
    try {
      ...
      console.error(err);
      setError(`Loglar yüklenirken hata oluştu: ${err.message || err}`);
    } finally {
  ...
  const copyToClipboard = (text: string) => {
    ...
    alert('Log detayları panoya kopyalandı.');
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="font-serif text-3xl text-[#ecd8a6]">Sistem Hata Logları</h2>
          <p className="text-sm text-[#ecd8a6]/60">Uygulama genelinde oluşan hataları ve yığın (stack trace) detaylarını izleyin.</p>
        </div>
        <div className="flex gap-3">
          ...
          >
            ...
            {isLive ? 'Canlı Yayında (Açık)' : 'Canlı İzle'}
          </button>
          <button
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3 rounded-xl border border-[#ecd8a6]/10 bg-[#0e0a1b] p-4">
        {/* Search */}
        <div>
          <label className="block text-xs uppercase tracking-wider text-[#ecd8a6]/60 mb-2 font-medium">Hata Arayın</label>
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Hata mesajı veya kod arayın..."
        ...
        {/* Date Filter */}
        <div>
          <label className="block text-xs uppercase tracking-wider text-[#ecd8a6]/60 mb-2 font-medium">Zaman Aralığı</label>
        ...
        {/* Sort */}
        <div>
          <label className="block text-xs uppercase tracking-wider text-[#ecd8a6]/60 mb-2 font-medium">Tarih Sıralaması</label>
          <button
      ...
      {loading ? (
        ...
        <div className="flex h-64 flex-col items-center justify-center rounded-xl border border-[#ecd8a6]/10 bg-[#0e0a1b]/40 text-[#ecd8a6]/40">
          ...
          <p className="mt-3 text-sm">Hata logu bulunamadı.</p>
        </div>
      ) : (
                ...
                >
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <p className="text-sm font-semibold truncate text-[#ecd8a6]/95">
                      {log.message || log.error || 'Bilinmeyen Hata'}
                    </p>
                  </div>
                ...
                {isExpanded && (
                  <div className="border-t border-[#ecd8a6]/10 bg-[#07040e]/60 p-6 space-y-4">
                    <div className="flex justify-between items-center">
                      <h4 className="text-xs uppercase tracking-wider text-[#ecd8a6]/50">Log Detayı (JSON)</h4>
                      <button
                        onClick={() => copyToClipboard(JSON.stringify(log, null, 2))}
                        className="flex items-center gap-1.5 rounded-lg border border-[#ecd8a6]/20 bg-purple-950/30 px-3 py-1 text-xs text-[#ecd8a6]/80 hover:bg-purple-950/50 transition"
                      >
