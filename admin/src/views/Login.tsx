  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (role === 'admin' || role === 'employee' || role === 'viewer') {
        ...
        await auth.signOut();
        setError('Bu panele erişim izniniz bulunmamaktadır.');
      }
    } catch (err: any) {
      console.error(err);
      if (err.code === 'auth/user-not-found' || err.code === 'auth/wrong-password' || err.code === 'auth/invalid-credential') {
        ...
        setError('Giriş yapılırken bir hata oluştu: ' + (err.message || err));
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-[#ecd8a6]/20 bg-[#0e0a1b]/80 p-8 shadow-2xl shadow-purple-950/20 backdrop-blur-xl">
        ...
        <form onSubmit={handleLogin} className="mt-8 space-y-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="relative flex w-full justify-center rounded-lg bg-gradient-to-r from-[#8a2be2]/80 to-[#ecd8a6]/80 px-4 py-3 text-sm font-semibold text-[#07040e] shadow-lg transition hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
          >
