...
interface StoreModalProps {
  ...
  user: User | null;
  language: string;
  storeTranslations: Record<string, Record<string, string>>;
...
export const StoreModal: React.FC<StoreModalProps> = ({
  ...
  const handleBuy = async (pack: { amount: number; price: string }) => {
    if (!user) return;
    try {
      showToast(language === 'tr' ? "Ödeme sayfasına yönlendiriliyorsunuz..." : "Redirecting to checkout...", 'info');
      ...
      if (!response.ok) {
        throw new Error(language === 'tr' ? "Ödeme oturumu başlatılamadı." : "Failed to create checkout session.");
      }
  ...
  const getTranslation = (dict: Record<string, Record<string, string>>, key: string, lang: string) => {
      ...
      >
        ...
        <div className="p-8 text-center pb-6 border-b border-[#ecd8a6]/10 relative overflow-hidden">
          ...
          <h2 className="text-2xl font-serif text-[#ecd8a6] tracking-widest uppercase">
            {getTranslation(storeTranslations, 'storeTitle', language)}
          </h2>
        </div>
            ...
            >
              {pack.popular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#ecd8a6] text-[#0a0512] text-[10px] font-bold px-3 py-0.5 rounded-full uppercase tracking-wider whitespace-nowrap">
                  {getTranslation(storeTranslations, 'popular', language)}
                </div>
              )}
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <div className="text-[#ecd8a6] font-serif flex items-baseline gap-1">
                    ...
                    <span className="text-sm opacity-80">{getTranslation(storeTranslations, 'moons', language)}</span>
                  </div>
                  {pack.bonus && (
